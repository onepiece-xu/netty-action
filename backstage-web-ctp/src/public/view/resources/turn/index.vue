    ...
    <div class="page-inner">
        ...
        <div class="table-content">
            <el-table ref="multipleTable" 
                ...
                <el-table-column prop="serverName" label="TURN服务名称" sortable>
                ...
                <!-- <el-table-column prop="stunUrl" label="STUN URL" width="200" sortable>
                    <template slot-scope="scope">
                         <span :title="scope.row.stunUrl">{{scope.row.stunUrl}}</span>
                    </template>
                </el-table-column>
                <el-table-column prop="turnTcpUrl" label="TURN URL(TCP)" sortable>
                    <template slot-scope="scope">
                        <span :title='scope.row.turnTcpUrl'>{{scope.row.turnTcpUrl}}</span>
                    </template> 
                </el-table-column>
                <el-table-column prop="turnUdpUrl" label="TURN URL(UDP)" sortable>
                    <template slot-scope="scope">
                        <span :title='scope.row.turnUdpUrl'>{{scope.row.turnUdpUrl}}</span>
                    </template> 
                </el-table-column>
                ...
                <el-table-column prop="credential" label="密码" sortable>
                    <template slot-scope="scope">
                        <span :title='scope.row.credential'>{{scope.row.credential}}</span>
                    </template> 
                </el-table-column> -->
                ...
                <el-table-column prop="publicPort" label="公网端口" sortable>
                    <template slot-scope="scope">
                        <span :title='scope.row.publicPort'>{{scope.row.publicPort}}</span>
                    </template> 
                </el-table-column>
                ...
                <el-table-column prop="detail" label="详情" >
                    <template slot-scope="scope">
                        <router-link v-bind:to="'/turn?id='+scope.row.id+'&name='+encodeURIComponent(scope.row.serverName)">
                            <span>查看</span>
                        </router-link>
                    </template>
                </el-table-column>
        ...
        <el-dialog
            ...
            <div class="control-group">
                ...
                <div class="controls">
                    <input type="text" class="form-control" v-model="turnForm.serverName" data-vv-as="服务器名称" name="serverName" v-validate="'required'">
                    <div class="error">
            ...
            <div class="control-group">
                ...
                <div class="controls">
                    <input type="text" class="form-control" v-model="turnForm.domain" data-vv-as="域名" name="domain" v-validate="'required'">
                    <div class="error">
            ...
            <div class="control-group">
                <label class="control-label">
                    <span class="red">*</span>STUN URL：
                </label>
                <div class="controls">
                    <input type="text" class="form-control" v-model="turnForm.stunUrl" data-vv-as="STUN URL" name="httpUrl" v-validate="'required'">
                    <div class="error">
                        <span v-show="errors.has('stunUrl')" class="error">{{ errors.first('stunUrl') }}</span>
                    </div>
                </div>
            </div>
            <div class="control-group">
                <label class="control-label">
                    <span class="red">*</span>TURN URL(TCP)：
                </label>
                <div class="controls">
                    <input type="text" class="form-control" v-model="turnForm.turnTcpUrl" data-vv-as="TURN URL(TCP)" name="turnTcpUrl" v-validate="'required'">
                    <div class="error">
                        <span v-show="errors.has('turnTcpUrl')" class="error">{{ errors.first('turnTcpUrl') }}</span>
                    </div>
                </div>
            </div>
            <div class="control-group">
                <label class="control-label">
                    <span class="red">*</span>TURN URL(UDP)：
                </label>
                <div class="controls">
                    <input type="text" class="form-control" v-model="turnForm.turnUdpUrl" data-vv-as="TURN URL(UDP)" name="turnUdpUrl" v-validate="'required'">
                    <div class="error">
                        <span v-show="errors.has('turnUdpUrl')" class="error">{{ errors.first('turnUdpUrl') }}</span>
                    </div>
                </div>
            </div>
            <div class="control-group">
                ...
                <div class="controls">
                    <input type="text" class="form-control" v-model="turnForm.userName" data-vv-as="账号" name="userName" v-validate="'required'">
                    <div class="error">
            ...
            <div class="control-group">
                ...
                <div class="controls">
                    <input type="text" class="form-control" v-model="turnForm.credential" data-vv-as="密码" name="credential" v-validate="'required'">
                    <div class="error">
                        <span v-show="errors.has('credential')" class="error">{{ errors.first('credential') }}</span>
                    </div>
                </div>
            </div>
            <div class="control-group">
                ...
                <div class="controls">
                    <input type="text" class="form-control" v-model="turnForm.publicIp"  name="publicIp">
                    <!-- <div class="error">
            ...
            <div class="control-group">
                <label class="control-label">公网端口：
                </label>
                <div class="controls">
                    <input type="text" class="form-control" v-model="turnForm.publicPort" name="publicPort">
                    <!-- <div class="error">
                        <span v-show="errors.has('publicPort')" class="error">{{ errors.first('publicPort') }}</span>
                    </div> -->
                </div>
            </div>
            <div class="control-group">
                ...
                <div class="controls">
                    <input type="text" class="form-control" v-model="turnForm.internalIp" name="internalIp">
                    <!-- <div class="error">
        ...
        <detail-page :props="hasRouteQuery">
            <turn-detail :detailProps="{id:$route.query.id,name:$route.query.name}" v-if='hasRouteQuery'></turn-detail>
        </detail-page>
    </div>
...
<script>
import {getTurnServerList,addTurnServer,updateTurnServer,deleteTurnServer,getbindList,getUnBindList,bindSignal} from '@/public/api/turn.js'
...
export default {
    ...
    methods:{
        ...
        initData(){
            ...
            getTurnServerList(oData).then((res) =>{
                console.log(res);
                if(res&&res.data){
        ...
        editItem(){
            this.type = "edit";
            this.addDialogVisible = true,
            this.addTitle = "编辑TURN服务器",
            this.turnForm =JSON.parse(JSON.stringify(this.multipleSelection[0]));
        ...
        commitModel(){
            let data = {
                "serverName":this.turnForm.serverName,
                "domain":this.turnForm.domain,
                "stunUrl":this.turnForm.stunUrl,
                "turnTcpUrl":this.turnForm.turnTcpUrl,
                "turnUdpUrl":this.turnForm.turnUdpUrl,
                "userName":this.turnForm.userName,
                "credential":this.turnForm.credential,
                "publicIp":this.turnForm.publicIp,
                "publicPort":this.turnForm.publicPort,
                "internalIp":this.turnForm.internalIp
            }
            this.$validator.validateAll().then((result)=>{
                if (result) {
                    this.addDialogVisible = false;
                    if(this.type == 'add'){
                        addTurnServer(data).then((res) =>{
                            console.log(res)
                            this.initData()
                        })
                    }else if(this.type == 'edit'){
                        data.id = this.turnForm.id;
                        updateTurnServer(data).then((res) =>{
                            console.log(res)
                            this.initData()
                        })
                    }
        ...
        confirm(type){
            ...
            case'del':
                ...
                let data = ids
                deleteTurnServer(data).then((res) =>{
                    console.log(res)
            ...
            case'bind':
                ...
                let data1 = {
                    "clusterIds":ids1,
                    "turnServer":JSON.parse(JSON.stringify(this.multipleSelection[0].id)),
                    "type":"bind"
                }
